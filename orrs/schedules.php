			...
			<table class="table table-bordered table-hover table-striped">
				...
				<tbody>
					<?php 
						$swhere = "";
						$i = 1;
						$courses = $conn->query("SELECT *,Concat(code,' - ',`name`) as course FROM `course_list` where id in (SELECT course_id FROM `schedule_list` where delete_flag = 0 {$swhere})");
                                ...
                                while($row = $qry->fetch_assoc()):
                                    $fc_capacity = isset($train_fcf_arr[$row['course_id']]) ? $train_fcf_arr[$row['course_id']] : 0;
                                    $e_capacity = isset($train_ef_arr[$row['course_id']]) ? $train_ef_arr[$row['course_id']] : 0;
					...
					?>
						<tr>
							...
							<td class="px-0"><?php echo isset($train_arr[$row['course_id']]) ? $train_arr[$row['course_id']] : "N/A" ?></td>
