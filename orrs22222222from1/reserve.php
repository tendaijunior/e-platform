<?php
if(isset($_GET['sid'])){
    $trains = $conn->query("SELECT *,Concat(code,' - ',`name`) as train FROM `course_list` where id in (SELECT course_id FROM `schedule_list` where delete_flag = 0 and id='{$_GET['sid']}')");
