        ...
        <div class="container-fluid">
			...
			<table class="table table-hover table-striped table-bordered">
				...
				<tbody>
					<?php 
						$i = 1;
						$trains = $conn->query("SELECT *,Concat(code,' - ',`name`) as train FROM `course_list` where id in (SELECT course_id FROM `schedule_list` where delete_flag = 0 and `type` = '{$type}')");
					...
					?>
						<tr>
							...
							<td class="px-0">
								<div class="px-1 border-bottom"><span class="text-muted">First Class:</span> <span class="text-muted fa fa-user"></span> <b><?= isset($train_fcf_arr[$row['course_id']]) ? $train_fcf_arr[$row['course_id']] : 0 ?></b> <span class="text-muted ml-2 fa fa-tag"></span> <b><?= rtrim(number_format($row['first_class_fare'],2),'.') ?></b></div>
								<div class="px-1"><span class="text-muted">Economy:</span> <span class="text-muted fa fa-user"></span> <b><?= isset($train_ef_arr[$row['course_id']]) ? $train_ef_arr[$row['course_id']] : 0 ?></b> <span class="text-muted ml-2 fa fa-tag"></span> <b><?= rtrim(number_format($row['economy_fare'],2),'.') ?></b></div>
							</td>
