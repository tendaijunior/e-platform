...
Class Master extends DBConnection {
	...
	function save_service(){
		$_POST['course_ids'] = implode(',',$_POST['course_ids']);
		extract($_POST);
		$data = "";
		foreach($_POST as $k =>$v){
			if(!in_array($k,array('id'))){
		...
		$check = $this->conn->query("SELECT * FROM `service_list` where `name` ='{$name}' and course_ids = '{$course_ids}' and delete_flag = 0 ".($id > 0 ? " and id != '{$id}' " : ""))->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function save_reservation(){
		...
		extract($_POST);
		$capacity = $this->conn->query("SELECT `".($seat_type == 1 ? "first_class_capacity" : "economy_capacity")."` FROM train_list where id in (SELECT course_id FROM `schedule_list` where id ='{$schedule_id}') ")->fetch_array()[0];
